...
if batch_size < total_data:   # Read all data once when using stochastic gradient descent
    ...
    # train_image = [train_image[i] for i in range(len(train_image)) if not i in range(int(fold_number*len(train_image)/5),int((fold_number+1)*len(train_image)/5))]
    # train_label = [train_label[i] for i in range(len(train_label)) if not i in range(int(fold_number*len(train_label)/5),int((fold_number+1)*len(train_label)/5))]
